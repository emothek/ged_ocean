...
router.post("/signup", tryCatch((req, res, next) => {
...
router.post("/login", tryCatch((req, res, next) => {
...
//refreshToken
router.post("/refreshToken", tryCatch(async (req, res, next) => {
...
router.get("/logout", auth, tryCatch(async (req, res, next) => {
  const { signedCookies = {} } = req;
...
router.delete("/user/:id", auth, tryCatch(async (req, res) => {
  let id = req.params.id;
...
router.post("/user/activity", auth, tryCatch(async (req,res)=>{
  let id=req.user.id;
  
  let activity=req.body.activity;
  if(activity === "userLoggedIn" || activity === "userLoggedOut" || activity === "userSignedUp")
  {
...
router.get("/activities",auth, tryCatch(async (req,res)=>{
  let user =req.user;
  if(user.role === "SUPERADMIN"){

    await prisma.activity.findMany({include:{user:true,file:true}}).then(activities => 
...
router.get("/profile", auth, tryCatch((req, res, next) => {
